...
- Zwei Indexierungs-Methoden für Artikel: **Intern** (Standard, schnell, nutzt REDAXOs eigenen Renderer) oder **HTTP Crawler** (ruft die Artikel-URL wie ein Browser ab – sinnvoll, wenn Cache/Proxy/Ausgabelogik den finalen Frontend-Inhalt gegenüber dem internen Renderer verändern, und der einzig mögliche Weg für externe Sitemap-URLs). Wiederkehrende Boilerplate wie Navigation/Header/Footer/Formulare wird beim Bereinigen automatisch entfernt (siehe `IndexerService::cleanText()`).
- **Eigene Bereiche gezielt von der Indexierung ausschließen**: ein Element mit der Klasse `aichatignore` (z. B. `<div class="aichatignore">…</div>`) wird beim Bereinigen komplett entfernt, egal wie tief verschachtelt oder auf welchem Tag – praktisch für Boilerplate, die kein passendes semantisches `<nav>`/`<footer>`-Tag trägt (z. B. ein Offcanvas-Menü). Kein CSS-Selektor zu konfigurieren, einfach die Klasse im eigenen Template setzen.
...
Ein Struktur-Bereich muss dabei kein konkreter Kategorie-Teilbaum sein: die Auswahl „🌐 Gesamte Struktur" erfasst stattdessen jeden Artikel der Installation, unabhängig von seiner Position im Kategoriebaum (auch Artikel ohne eigene Kategorie, z. B. Impressum/Datenschutz) – sie kann pro Profil nur in maximal einer Zeile gleichzeitig gewählt werden. Jeder Struktur-Bereich (egal ob „Gesamte Struktur" oder eine konkrete Kategorie) hat außerdem eine eigene Option **„Auch Offline-Inhalte einschließen"** (Standard: an, damit sich das Verhalten bestehender Profile nicht rückwirkend ändert): ist sie deaktiviert, werden nur Online-Artikel dieses Bereichs indexiert. Unabhängig davon gilt für Struktur-Bereiche weiterhin zusätzlich die yrewrite-„noindex"-SEO-Einstellung eines Artikels (siehe „Ausschlüsse und Datenschutz" unten). Die Kategorie-Auswahl selbst unterstützt eine Live-Suche (praktisch bei vielen Kategorien) und markiert Kategorien, die der Mountpoint (Startkategorie) einer konfigurierten yrewrite-Domain sind, direkt mit einem Badge – hilfreich bei mehreren Domains, um auf einen Blick zu sehen, welcher Teilbaum zu welcher Domain gehört.
...
- Ein einzelner Artikel innerhalb eines sonst indexierten Struktur-Bereichs lässt sich über yrewrites eigene SEO-Einstellung „nicht indexieren" (pro Artikel) ausschließen, sofern „yrewrite-SEO-Einstellungen respektieren" aktiv ist (Einstellungen → Indexierungs-Quellen, Standard: an) – praktisch, um z. B. eine einzelne Seite ohne eigenes Profil-Redesign aus dem Suchmaschinen- *und* KI-Index herauszuhalten. Diese Regel gilt unabhängig von der „Auch Offline-Inhalte einschließen"-Option des jeweiligen Struktur-Bereichs immer zusätzlich. Der Online-/Offline-Status eines Artikels wirkt sich nur aus, wenn diese Option für den betreffenden Struktur-Bereich explizit deaktiviert wurde (Standard: an, also weiterhin status-unabhängig – ein einmal als Wissensquelle gewählter Struktur-Bereich indexiert dann unabhängig davon vollständig).
...
- **PHP ≥ 8.4** (Mindestanforderung laut `package.yml`) - nutzt die spec-konforme `Dom\HTMLDocument`/`Dom\XPath`-API aus PHP 8.4 (z.B. für die `aichatignore`-Konvention, siehe „Indexierung" oben), keine ältere PHP-Version unterstützt. Entwickelt/getestet wird gegen PHP 8.5.
