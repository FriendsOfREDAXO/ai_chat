...
if ($profiles === []) {
    ...
    foreach ($profiles as $profileId => $profile) {
        ...
        $profileRowsHtml .= '<div class="row" style="margin-top:10px;"><div class="col-md-3"><label>URL-Modus</label><select class="form-control" name="profiles[' . rex_escape((string) $profileId) . '][url_mode]">' . $renderOptions([
            'field' => 'Aus Feldwert',
            'profile' => 'URL-Profil (Namespace)',
            'template' => 'Template',
        ], (string) ($profile['url_mode'] ?? 'field'), '— Modus —', false) . '</select></div>';
        $profileRowsHtml .= '<div class="col-md-3"><label>URL-Feld</label>' . $renderColumnSelect((string) $profileId, 'url_field', (string) ($profile['url_field'] ?? ''), $profileTable, $columnsMap, true, '— optional —') . '</div>';
        $profileRowsHtml .= '<div class="col-md-3"><label>URL-Profil</label><input class="form-control" type="text" name="profiles[' . rex_escape((string) $profileId) . '][url_profile]" value="' . rex_escape((string) ($profile['url_profile'] ?? '')) . '" placeholder="news"></div>';
        $profileRowsHtml .= '<div class="col-md-3"><label>URL-Template</label><input class="form-control" type="text" name="profiles[' . rex_escape((string) $profileId) . '][url_template]" value="' . rex_escape((string) ($profile['url_template'] ?? '')) . '" placeholder="/news/{id}-{slug}"></div></div>';
...
$templateHtml .= '<div class="row" style="margin-top:10px;"><div class="col-md-3"><label>URL-Modus</label><select class="form-control" name="profiles[' . $templateProfileKey . '][url_mode]"><option value="field" selected>Aus Feldwert</option><option value="profile">URL-Profil (Namespace)</option><option value="template">Template</option></select></div><div class="col-md-3"><label>URL-Feld</label><select class="form-control js-column-select" data-profile-key="' . $templateProfileKey . '" name="profiles[' . $templateProfileKey . '][url_field]"><option value="">— optional —</option></select></div><div class="col-md-3"><label>URL-Profil</label><input class="form-control" type="text" name="profiles[' . $templateProfileKey . '][url_profile]" value="" placeholder="news"></div><div class="col-md-3"><label>URL-Template</label><input class="form-control" type="text" name="profiles[' . $templateProfileKey . '][url_template]" value="" placeholder="/news/{id}-{slug}"></div></div>';
